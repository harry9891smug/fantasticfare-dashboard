...
interface Day {
  ...
  activity_images: File[] // for new uploads
...
export default function EditItinerary() {
  ...
  const [activity_id, setActivityId] = useState<string | null>(null);
  const [itinerary, setActivity] = useState<Day[]>([{
    day_name: '',
    day_activities: '',
    activity_type: 'solo',
    activity_images: [],
    ...
    const fetchItinerary = async () => {
      ...
      try {
        ...
        if (response.data.data.activities && response.data.data.activities.length > 0) {
          setActivityId(response.data.data.activities[0]._id)
          const existingActivity: Day[] = response.data.data.activities[0].days.map((day: any) => ({
            day_name: day.day_name || '',
            day_activities: day.day_activities || '',
            activity_type: day.activity_type || 'solo',
  ...
  const handleFileChange = (index: number, e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      updated[index] = {
        ...updated[index],
        activity_images: [...updated[index].activity_images, ...files],
  ...
  const removeNewImage = (dayIndex: number, imageIndex: number) => {
    ...
    updated[dayIndex].activity_images.splice(imageIndex, 1)
    ...
    <div className="container-fluid">
      <div className="card">
        <div className="card-header">
          <h5>Edit Activity for Package #{id}</h5>
        </div>
              ...
              <div key={index} className="mb-4 border p-3 rounded">
                ...
                <div className="row">
                  ...
                  <div className="col-sm-4">
                    ...
                    <select
                      className="form-control"
                      value={day.activity_type}
                      onChange={(e) => handleDayChange(index, 'activity_type', e.target.value)}
                    >
                ...
                <div className="mt-2">
                  ...
                  <textarea
                    className="form-control"
                    rows={3}
                    value={day.day_activities}
                    onChange={(e) => handleDayChange(index, 'day_activities', e.target.value)}
                    required
                  />
                </div>
